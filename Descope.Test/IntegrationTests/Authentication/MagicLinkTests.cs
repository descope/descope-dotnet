...
namespace Descope.Test.Integration
{
    ...
    public class MagicLinkTests
    {
        ...
        public MagicLinkTests()
        {
            ...
            _configuration = JsonDocument.Parse(json);
        }
        ...
        {
            var appSettings = _configuration.RootElement.GetProperty("AppSettings");
            var magicLink = appSettings.GetProperty("MagicLink");
            var scenariosArray = magicLink.GetProperty("Scenarios");

            foreach (var scenarioElement in scenariosArray.EnumerateArray())
            {
                var scenarioName = scenarioElement.GetProperty("name").GetString();
                if (scenarioName == name)
                {
                    return new MagicLinkScenario
                    {
                        Name = scenarioName ?? "",
                        Email = scenarioElement.GetProperty("email").GetString() ?? "",
                        RedirectUrl = scenarioElement.TryGetProperty("redirectUrl", out var redirProp) ? redirProp.GetString() ?? "" : ""
                    };
                }
            }
        ...
        public async Task MagicLink_SignIn_Email_Success()
        {
            ...
            if (string.IsNullOrEmpty(redirectUrl))
            {
                ...
                try { await _descopeClient.Mgmt.V1.User.DeletePath.PostAsync(new DeleteUserRequest { Identifier = testLoginId }); } catch { }
                ...
                var signInRequest = new MagicLinkSignInRequest
                {
                    LoginId = testLoginId,
                    RedirectUrl = "https://example.com/auth"
                };
                await _descopeClient.Auth.V1.Magiclink.Signin.Email.PostAsync(signInRequest);
            ...
            else
            {
                ...
                var searchRequestVerify = new SearchUsersRequest { UserIds = new List<string> { authResponse.User.UserId! } };
                var searchResultVerify = await _descopeClient.Mgmt.V2.User.Search.PostAsync(searchRequestVerify);
                Assert.NotNull(searchResultVerify);
                Assert.Single(searchResultVerify.Users!);
                ...
                // Cleanup user at end
                await _descopeClient.Mgmt.V1.User.DeletePath.PostAsync(new DeleteUserRequest { Identifier = testLoginId });
            }
        }
        ...
        public async Task MagicLink_SignUp_Email_Success()
        {
            ...
            if (string.IsNullOrEmpty(redirectUrl))
            {
                ...
                var signUpRequest = new MagicLinkSignUpEmailRequest
                {
                    LoginId = testLoginId,
                    Email = testLoginId,
                    RedirectUrl = "https://example.com/auth",
                ...
                await _descopeClient.Auth.V1.Magiclink.Signup.Email.PostAsync(signUpRequest);
        ...
        public async Task MagicLink_SignUpOrIn_Email_Success()
        {
            ...
            if (string.IsNullOrEmpty(redirectUrl))
            {
                ...
                // Note: SignUpOrIn uses the same request type as SignIn (MagicLinkSignInRequest)
                // but the SignUpOrIn endpoint will create a user if they don't exist
                var signUpOrInRequest = new MagicLinkSignInRequest
                {
                    LoginId = testLoginId,
                    RedirectUrl = "https://example.com/auth"
                };

                await _descopeClient.Auth.V1.Magiclink.SignupIn.Email.PostAsync(signUpOrInRequest);
