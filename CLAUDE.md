...
make add-mgmt ENDPOINT=/v1/mgmt/foo   # Add one endpoint without a full regen (see README-maintainer.md)
...
- `Descope/Generated/Mgmt/` and `Descope/Generated/Auth/` — **Auto-generated by Kiota. Never edit manually**, with one documented exception: a targeted endpoint addition copies new generated files in and merges a single navigation property into one existing request builder. Use the `kiota-add-endpoint` skill and see "Targeted Endpoint Additions" in `README-maintainer.md`. Otherwise regenerated by `make generate`.
