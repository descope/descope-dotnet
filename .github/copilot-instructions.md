...
- One documented exception: a targeted endpoint addition (`make add-mgmt` / `make add-auth`) copies new generated files in and merges a single navigation property into one existing request builder. See "Targeted Endpoint Additions" in `README-maintainer.md`
...
# Add one endpoint without a full regen, then merge (see README-maintainer.md)
make add-mgmt ENDPOINT=/v1/mgmt/foo
...
- Never edit generated files directly, except the documented merge step of a targeted endpoint addition
